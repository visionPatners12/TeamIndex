...
import { recalculateOfficialPriceForPool } from "../services/priceEngine";
...
export async function syncLimitlessPortfolioForPool(env: Env, poolId: string) {
  ...
  const fetchStep = async <T>(step: "positions" | "history" | "pnl", fn: () => Promise<T>) => {
    try {
      return await fn();
    } catch (cause) {
  ...
  const { valuation, valuationSnapshot } = await recalculateOfficialPriceForPool(env, poolId, {
    valuationSnapshot: {
