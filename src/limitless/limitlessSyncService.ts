...
async function setSyncStatus(
  ...
  await (prisma as any).limitless_sync_state.upsert({
    where: { id: "default" },
...
export async function syncCategories(env: Env): Promise<number> {
  const raw = await listCategories(env);
  let upserted = 0;
  ...
  for (const cat of raw) {
    ...
    await (prisma as any).limitless_categories.upsert({
      where: { id: cat.id },
      update: { slug, label, rawJson: cat as object, updatedAt: new Date() },
      create: { id: cat.id, slug, label, rawJson: cat as object },
    });
    upserted++;
  }
 ...
 */
export async function syncMarkets(env: Env): Promise<number> {
  ...
  try {
    ...
    cursor = undefined;
    while (true) {
      const page = await listActiveMarkets(env, { limit: 100, cursor });
      const markets: LimitlessMarket[] = page.data ?? [];
...
async function upsertMarketBatch(markets: LimitlessMarket[]) {
  const now = new Date();
  ...
  for (const m of markets) {
    ...
    const { yesPrice, noPrice } = extractPrices(m);
    ...
    await (prisma as any).limitless_markets.upsert({
      where: { id: m.slug },
      update: {
 ...
 */
export async function syncPrices(env: Env, limit = 200): Promise<number> {
  const activeMarkets: Array<{ id: string }> = await (prisma as any).limitless_markets.findMany({
    where: { status: "ACTIVE" },
    select: { id: true },
  ...
  for (const { id: marketSlug } of activeMarkets) {
    ...
    const series = await getHistoricalPrices(env, marketSlug);
    ...
    for (const outcome of series) {
      ...
      for (const tick of outcome.prices ?? []) {
        ...
        try {
          await (prisma as any).limitless_prices.upsert({
            where: {
 ...
 */
export async function enrichSportsGames(env: Env, limit = 500): Promise<number> {
  // Process markets that don't have a lim_games row yet
  const markets: Array<{ id: string; title: string; rawJson: unknown }> = await (prisma as any).limitless_markets.findMany({
    where: {
  ...
  for (const m of markets) {
    const raw = (m.rawJson ?? {}) as LimitlessMarket;
    const hints = detectSportHints({ ...raw, slug: m.id, title: m.title });
    if (!hints) continue;

    const gameTime: Date | null = (() => {
      const t = (raw as any).expirationDate ?? (raw as any).gameStartTime ?? null;
      if (!t) return null;
      const d = new Date(t);
 ...
 */
export async function runFullLimitlessSync(env: Env): Promise<{
  categories: number;
  markets: number;
  prices: number;
  games: number;
