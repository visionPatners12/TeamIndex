...
async function executeTranche(params) {
    ...
    try {
        ...
        const readiness = await (0, polymarketWallet_1.getPolymarketReadiness)(env, candidate.tokenId);
        ...
        let orderResult;
        try {
            orderResult = await (0, clobClient_1.postLimitOrder)(env, {
        ...
        if (!(0, clobClient_1.isAcceptedOrderResult)(orderResult)) {
            const lastError = (0, clobClient_1.getOrderRejectMessage)(orderResult);
            await finishQueue(queue.id, "FAILED", lastError);
